  ...
  v-container(grid-list text-xs-center)
    v-layout(row wrap)
      v-flex
        table.ma-a.text-xs-center
          tbody.black
            tr
              ...
              td(:class="{ black: algae, green: !algae }").darken-4 {{ desiredChange }}
            ...
            tr
              ...
              td(:class="{'border--right': !algae }").border--left 
              td(:class="{ black: algae, red: !algae }").darken-4 {{ changeProvided }}
              td(:class="{'border--left': !algae }")
              td 
            tr.slim
              td.border--bottom 
              ...
              td(:class="{ black: algae, grey: !algae, 'black--text': algae, 'border--bottom': algae }").darken-2 &#61;
            ...
            tr
              ...
              td.grey.darken-2 X
              td(:class="{'hide--water-factor': problemUse && (attempts < 2 || !correctState)}").purple.darken-4 {{ waterFactor }}
              td(:class="{ 'hide--math-operator': algae }").grey.darken-2 X
              td(:class="{ grey: algae, 'darken-2': algae, orange: !algae, 'darken-4': !algae }") {{ chemicalFactor }}
              td.grey.darken-2 &#61;
        ...
        p.text-xs-center.mt-2(v-if="conversion && correctState") {{ conversion }}
</template>
...
export default {
  ...
  props: ['poolVolume', 'desiredChange', 'unitVolume', 'changeProvided', 'chemicalAmount', 'chemicalType', 'waterFactor', 'chemicalFactor', 'total', 'conversion', 'algae', 'problemUse'],
  computed: {
...
<style scoped>
  ...
  /* give the Total cell the name with as the Chemical Amount cell */
  ...
  .hide--pool-volume { color: #0d47a1; }
  .hide--chemical-cells { color: #b71c1c; }
  .hide--water-factor { color: #4a148c; }
  .hide--math-operator { color: #616161; }
